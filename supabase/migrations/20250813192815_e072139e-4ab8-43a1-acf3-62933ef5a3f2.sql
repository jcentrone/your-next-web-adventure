...
-- First, let's check if the column already exists and handle existing data
ALTER TABLE section_guidance 
ALTER COLUMN "infoFields" TYPE jsonb 
USING CASE 
  WHEN "infoFields" IS NULL THEN NULL
  WHEN jsonb_typeof("infoFields"::jsonb) = 'array' THEN "infoFields"::jsonb
  ELSE jsonb_build_array()
END;

-- Insert structured data for roof section with proper JSON syntax
