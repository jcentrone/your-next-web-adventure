...
export function addTriangle(canvas: FabricCanvas, palette: Palette, x = 100, y = 100) {
    const tri = new Polygon(
        ...
        {left: x, top: y, fill: palette.colors[0], stroke: palette.colors[1] || palette.colors[0], strokeWidth: 2}
    );
    canvas.add(tri);
...
export function addArrow(canvas: FabricCanvas, palette: Palette, x = 100, y = 100) {
    ...
    const g = new Group([line, head], {left: x, top: y});
    canvas.add(g);
...
export function addBidirectionalArrow(canvas: FabricCanvas, palette: Palette, x = 100, y = 100) {
    ...
    const g = new Group([line, headL, headR], {left: x, top: y});
    canvas.add(g);
    canvas.setActiveObject(g);
    canvas.requestRenderAll();
    return g;
...
export function addText(canvas: FabricCanvas, palette: Palette, text = "Text", x = 120, y = 120) {
    const tb = new Textbox(text, {left: x, top: y, fontSize: 24, fill: palette.colors[3] || palette.colors[0]});
    canvas.add(tb);
        ...
        await new Promise<void>(resolve => {
            loadSVGFromString(svg, (objects, options) => {
                if (objects) {
                    ...
                    obj.set({left: x, top: y, stroke, fill: "none"});
                    canvas.add(obj);
