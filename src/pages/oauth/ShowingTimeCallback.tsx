            ...
            try {
                const res = await fetch("https://api.showingtime.com/oauth/token", {
                    ...
                    body: new URLSearchParams({
                        ...
                        redirect_uri: `${window.location.origin}/oauth/showingtime`,
                        grant_type: "authorization_code",
                    }),
                });
                if (res.ok) {
                    const token = await res.json();
                    await showingTime.handleOAuthCallback(state, token);
                    queryClient.invalidateQueries({
                        queryKey: ["showingtime-connected", state],
                    });
                }
            } finally {
                navigate("/settings/integrations");
...
export default ShowingTimeCallback;
