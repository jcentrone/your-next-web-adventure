...
import type { TxCoastalWindstormMitigationReport } from "@/lib/reportSchemas";
import { TX_WINDSTORM_QUESTIONS } from "@/constants/txWindstormQuestions";

interface EditorProps {
  report: TxCoastalWindstormMitigationReport;
  onUpdate: (r: TxCoastalWindstormMitigationReport) => void;
}

const sectionSchema = TX_WINDSTORM_QUESTIONS.sections.reduce((acc, section) => {
...
const TxWindstormEditor: React.FC<EditorProps> = ({ report, onUpdate }) => {
  ...
  const handleSave = async () => {
    try {
      ...
      const updated = { ...report, reportData: current } as TxCoastalWindstormMitigationReport;
      await dbUpdateReport(updated);
      onUpdate(updated);
    ...
    <div className="space-y-6">
      ...
      <Form {...form}>
        <div className="space-y-8">
          {TX_WINDSTORM_QUESTIONS.sections.map((section) => (
...
export default TxWindstormEditor;
