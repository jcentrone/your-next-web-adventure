...
const tourOptions = {
  defaultStepOptions: {
    classes: 'shadow-md bg-card text-card-foreground rounded-lg',
    scrollTo: true,
...
const OnboardingTour: React.FC<OnboardingTourProps> = ({ onComplete }) => {
  const { markUserAsReturning } = useAuth();
  ...
  useEffect(() => {
    ...
    },
    {
      ...
      `,
      attachTo: { element: '[role="button"]:has([alt="User avatar"])', on: 'left' as const },
      buttons: [
        {
          text: 'Previous',
          action: function() {
...
}, [markUserAsReturning, onComplete]);
